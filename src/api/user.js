            ...
            .then(usr => {
              ephemeral
                ...
                .then(pass => {
                  ...
                  mkdir(`/home/git/.gitolite/keydir/${usr.id}`, err => {
                    if (err) {
                      return emit.reject("user.create", client, "500", err);
                    }
                  });
                  return emit.resolveWithData(
                    "user.create",
                    client,
                    "200",
                    post,
                ...
                .catch(() =>
                  emit.reject("user.create", client, "500", "server error")
                );
            })
            .catch(err => {
  ...
  security
    ...
    .then(() => {
      db.user
        ...
        .then(user => {
          if (!user) {
            return emit.reject("user.delete", client, "404", "user not found");
          }
          if (user.type === "superadmin") {
            return emit.reject(
              ...
              client,
              "401",
              "superadmin can\t be deleted"
            );
          }
          db.contact
            .destroy({
              where: {
                [db.Or]: [{ user_id: user.id }, { contact_id: user.id }]
              }
            })
            .then(() => {
              db.device
                      ...
                      db.repo
                        ...
                        .then(() => {
                          user
                            ...
                            .catch(err =>
                              emit.reject("user.delete", client, "500", err)
                            );
                        })
                        .catch(err =>
                          emit.reject("user.delete", client, "500", err)
...
function connect(client, msg) {
  if (
    !msg.email ||
    !msg.password ||
    (msg.token !== true && msg.token !== false) ||
    !msg.sessionId ||
    !msg.sessionName ||
    !msg.sessionType
  ) {
  ...
  db.user
    ...
    .then(user => {
      if (!user) {
        return emit.reject("user.connect", client, "404", "user not found");
      }
      security
...
function disconnect(client) {
  security
    ...
    .then(data => {
      tokenManagement.remove(data.sessionId, data.id).then(() => {
        userManagement
          ...
          db.user
            ...
            .then(user => {
              ...
              user
                ...
                .catch(err =>
                  emit.reject("user.updatePassword", client, "500", err)
                );
            })
            .catch(err =>
              emit.reject("user.updatePassword", client, "500", err)
      ...
      db.user
        ...
        .then(user => {
          ...
          user
            ...
            .then(() =>
              emit.resolve("user.updateEmail", client, "200", "login updated")
            )
...
export default function run(client) {
  ...
  client.on("user.delete", msg => {
    Delete(client, dialogue.convert(msg));
  });

  client.on("user.connect", msg => {
    connect(client, dialogue.convert(msg));
  });
  client.on("user.disconnect", () => {
    disconnect(client);
  });

  client.on("user.getData", () => {
    getData(client);
  });

  client.on("user.updatePassword", msg => {
    updatePassword(client, dialogue.convert(msg));
  });
  client.on("user.updateIcon", msg => {
    updateIcon(client, dialogue.convert(msg));
  });
  client.on("user.updateLogin", msg => {
    updateLogin(client, dialogue.convert(msg));
  });
  client.on("user.updateEmail", msg => {
    updateEmail(client, dialogue.convert(msg));
  });
}
